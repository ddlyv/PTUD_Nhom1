...
<body>
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $info = $bacsi->layThongTinBenhNhan($id);
    } else {
    ...
    if (isset($_POST['btn_update'])) {
        ...
        $diachi = $_POST['diachi'];
        $trangThai = $_POST['trangThai']=='inactive' ? 'Bệnh nhân Khóa' : 'Bệnh nhân';
        $maTaiKhoan = $_POST['maTaiKhoan'];

        $bacsi->updateBenhNhan($info['maBenhNhan'], $hoTenDem, $ten, $email, $diachi, $soDienThoai, $date, $gioiTinh);
        $result=$bacsi->updateTrongTaiKhoan($maTaiKhoan , $hoTenDem , $ten , $soDienThoai,$trangThai);
        if ($result) {
            echo "<script>alert('Cập nhật thông tin bệnh nhân thành công!'); window.location.href='quan_li_tai_khoan_benh_nhan.php';</script>";
        } else {
            echo "<script>alert('Cập nhật thông tin bệnh nhân thất bại!');</script>";
        }
    }
    ?>

    <div class="container mt-5">
        <h2 class="text-center mb-4">Sửa Thông Tin Bệnh Nhân</h2>

        <form id="formUpdate" method="POST" enctype="multipart/form-data">
            <input type="hidden" name="id" value="<?php echo htmlspecialchars($info['maBenhNhan']); ?>">
            ...
            <a href="quan_li_tai_khoan_nhan_vien.php" class="btn btn-secondary">Hủy</a>
        </form>
    </div>
